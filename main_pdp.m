...
pic_num = 1;
for kk = 1:1:40
    sgtitle(sprintf("Illurstration of ray-tracing at %.1fs",dt*kk));
    % Plot environment
    subplot(1,2,1);
...
%     view(2)
    ...
    for ii = 1:1:length(ray_connections) 
        ...
        if flag1 == 0
            if size(connections,1) == 1
                ...
                for jj = 1:1:size(connections,1)
                    NLoS = plot3([connections(jj,1) connections(jj,4)],[connections(jj,2) connections(jj,5)],[connections(jj,3) connections(jj,6)],'Color',blue,'LineWidth',1);
                end
            end
            ray_index = [ray_index ii];
        end
    end
    ...
    axis equal
    try
        legend([lgd_tx, lgd_rx, LoS, NLoS],'Tx','Rx', 'LoS', 'NLoS','Location','northeast');
    catch err
        legend([lgd_tx, lgd_rx, NLoS],'Tx','Rx', 'NLoS','Location','northeast');
    end
%     axis off;
    ...
    hold off;
    if (ray_index(1)==1)
        LoS_pdp = stem(1e9*ray_matrix(1,1),10*log10(ray_matrix(1,2)),'Color',yellow,'LineWidth',2);
        hold on;
        NLoS_pdp = stem(1e9*ray_matrix(ray_index(2:end),1),10*log10(ray_matrix(ray_index(2:end),2)),'Color',blue,'LineWidth',1);
        legend([LoS_pdp,NLoS_pdp],'LoS','NLoS','Location','northeast');
    else
        NLoS_pdp = stem(1e9*ray_matrix(ray_index,1),10*log10(ray_matrix(ray_index,2)),'Color',blue,'LineWidth',1);
        legend([NLoS_pdp],'NLoS','Location','northeast');
    end
    ax = gca;
    set(ax, 'YDir', 'reverse');
    xlim([0 max(ray_matrix(:,1))*1e9+10]);
    ylim([-70 0]);
